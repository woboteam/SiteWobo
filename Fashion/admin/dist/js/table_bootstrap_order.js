...
// Xử lý nút cho phép hiển thị đơn hàng
$("#dataTables_products").on("click","a.btn-github",function(e){
    var rpl = $(this);
    var id = $(this).attr("data-id");
    $.ajax({
        type: "POST",
        url: "process_show_order.php",
        data: {id: id},
...
// Xử lý nút không cho hiển thị đơn hàng
$("#dataTables_products").on("click","a.btn-bitbucket",function(e){
    var rpl = $(this);
    var id = $(this).attr("data-id");
    $.ajax({
        type: "POST",
        url: "process_hide_order.php",
        data: {id: id},
        success: function(response){
            if (response==="success"){
...
// Xử lý nút hiển thị chi tiết đơn hàng
$("#model_products_detail").on("show.bs.modal", function(e) {
    var link = $(e.relatedTarget);
    $(this).find(".modal-body").load("load_order_detail.php?id="+link.attr("data-id")); 
});
...
$(document).ready(function (){
   ...
   // Đánh dấu ẩn các đơn hàng đã chọn
   $("#submit_unchecked_selected").click(function(e){
        var sl = table.rows(".selected",{page:'current'}).count();
        if (sl===0){
            bootbox.alert("Chọn ít nhất một đơn hàng");
        } else {
          bootbox.confirm({ 
            message: "Bạn chắc chắn là muốn không hiển thị các đơn hàng đã chọn này?", 
            buttons: {
                confirm: {
                    label: 'Đồng ý',
                      ...
                      success: function(response){
                        ...
                        if (response==="success"){
                            table.rows(".selected",{page:'current'}).every(function(rowIdx, tableLoop, rowLoop){
                                ...
                                cell.data("<a class='btn btn-social-icon btn-github m-t-2 m-r-1' title='Không hiển thị' data-id='"+id+"'><i class='fa fa-eye-slash'></i></a> <a class='btn btn-social-icon btn-linkedin m-t-2 m-r-1' title='Chi tiết đơn hàng' data-toggle='modal' data-target='#model_products_detail' data-id='"+id+"'><i class='fa fa-pencil-square-o'></i></a> <a class='btn btn-social-icon btn-warning m-t-2 m-r-1' title='Sửa thông tin đơn hàng' href='edit_product.php?id="+id+"'><i class='fa fa-pencil'></i></a> <a class='btn btn-social-icon btn-pinterest m-t-2 m-r-1' title='Xóa đơn hàng' data-id='"+id+"'><i class='fa fa-trash-o'></i></a>");
   ...
   // Đánh dấu hiện các đơn hàng đã chọn
   $("#submit_checked_selected").click(function(e){
        ...
        if (sl===0){
          ...
          bootbox.confirm({ 
            message: "Bạn chắc chắn là muốn hiển thị các đơn hàng đã chọn này?", 
            buttons: {
                confirm: {
                    label: 'Đồng ý',
                    className: 'btn-danger'
                      ...
                      success: function(response){
                          ...
                          if (response==="success"){
                              table.rows(".selected", {page:'current'}).every(function(rowIdx, tableLoop, rowLoop){
                                  ...
                                  var id = a[0];
                                  var cell = table.cell(this,4);
                                  cell.data("<a class='btn btn-social-icon btn-bitbucket m-t-2 m-r-1' title='Hiển thị' data-id='"+id+"'><i class='fa fa-eye'></i></a> <a class='btn btn-social-icon btn-linkedin m-t-2 m-r-1' title='Chi tiết đơn hàng' data-toggle='modal' data-target='#model_products_detail' data-id='"+id+"'><i class='fa fa-pencil-square-o'></i></a> <a class='btn btn-social-icon btn-warning m-t-2 m-r-1' title='Sửa thông tin đơn hàng' href='edit_product.php?id="+id+"'><i class='fa fa-pencil'></i></a> <a class='btn btn-social-icon btn-pinterest m-t-2 m-r-1' title='Xóa đơn hàng' data-id='"+id+"'><i class='fa fa-trash-o'></i></a>");
                                  this.invalidate().draw(false);
                              })
                          } else {
                              bootbox.alert("Error data: Không thực thi được. Liên hệ với admin");
   ...
   $("#submit_delete_selected").click(function(e){
        ...
        if (sl===0){
            ...
            bootbox.confirm({ 
                message: "Bạn chắc chắn là xóa tất cả các đơn hàng đã chọn trong trang này?",
                buttons: {
                    confirm: {
                        label: 'Đồng ý',
                        className: 'btn-danger'
