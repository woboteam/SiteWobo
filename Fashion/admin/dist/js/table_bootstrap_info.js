...
$("#dataTables_pages").on("click","a.btn-github",function(e){
    var rpl = $(this);
    var id = $(this).attr("data-id");
    $.ajax({
        type: "POST",
        url: "process_show_pages.php",
        data: {id: id},
...
$("#dataTables_pages").on("click","a.btn-bitbucket",function(e){
    var rpl = $(this);
    var id = $(this).attr("data-id");
    $.ajax({
        type: "POST",
        url: "process_hide_pages.php",
        data: {id: id},
        success: function(response){
            if (response==="success"){
...
$("#model_pages_detail").on("show.bs.modal", function(e) {
    var link = $(e.relatedTarget);
    $(this).find(".modal-body").load("load_infor_detail.php?id="+link.attr("data-id")); 
});

$(document).ready(function (){
   // Array holding selected row IDs
   var rows_selected = [];
   var table = $('#dataTables_pages').DataTable({
      'ajax': 'load_info.php',
      'columnDefs': [{
         'targets': 0,
         'searchable':false,
   ...
   $('#dataTables_pages').on('click', 'tbody td:not(:last-child), thead th:first-child', function(e){
   ...
   $('thead input[name="select_all"]', table.table().container()).on('click', function(e){
      if(this.checked){
         $('#dataTables_pages tbody input[type="checkbox"]:not(:checked)').trigger('click');
      } else {
         $('#dataTables_pages tbody input[type="checkbox"]:checked').trigger('click');
      }
          ...
          bootbox.confirm({ 
            ...
            callback: function(result){
                if (result){
                  ...
                  $.ajax({
                      type: "POST",
                      url: "process_hide_pages_selected.php",
                      data: {id: list_id},
                      success: function(response){
                        $(".loader").fadeOut("slow");
                        if (response==="success"){
          ...
          bootbox.confirm({ 
            ...
            callback: function(result){
                if (result){
                  ...
                  $.ajax({
                      type: "POST",
                      url: "process_show_pages_selected.php",
                      data: {id: list_id},
                      success: function(response){
                          $(".loader").fadeOut("slow");
                          if (response==="success"){
                              table.rows(".selected", {page:'current'}).every(function(rowIdx, tableLoop, rowLoop){
                                  ...
                                  var id = a[0];
                                  var cell = table.cell(this,4);
                                  cell.data("<a class='btn btn-social-icon btn-bitbucket m-t-2 m-r-1' title='Hiển thị' data-id='"+id+"'><i class='fa fa-eye'></i></a> <a class='btn btn-social-icon btn-linkedin m-t-2 m-r-1' title='Chi tiết sản phẩm' data-toggle='modal' data-target='#model_pages_detail' data-id='"+id+"'><i class='fa fa-pencil-square-o'></i></a> <a class='btn btn-social-icon btn-warning m-t-2 m-r-1' title='Sửa thông tin sản phẩm' href='edit_product.php?id="+id+"'><i class='fa fa-pencil'></i></a> <a class='btn btn-social-icon btn-pinterest m-t-2 m-r-1' title='Xóa sản phẩm' data-id='"+id+"'><i class='fa fa-trash-o'></i></a>");
                                  this.invalidate().draw(false);
                              })
                          } else {
                              bootbox.alert("Error data: Không thực thi được. Liên hệ với admin");
   ...
   $("#model_pages_detail_selected").on("show.bs.modal", function(e) {
        var md = $(this);
        var list_id = [];
        table.rows(".selected",{page:'current'}).every(function(rowIdx, tableLoop, rowLoop){
            var r = this.data();
            ...
            bootbox.confirm({ 
                ...
                callback: function(result){
                    if (result){
                        ...
                        $.ajax({
                            type: "POST",
                            url: "process_delete_pages_selected.php",
                            data: {id: list_id},
                            success: function(response){
                                $(".loader").fadeOut("slow");
                                // alert(response);
                                if (response==="success"){
                                    table
                                        ...
                                        .remove()
                                        .draw(false);
                                    $(".view_count_pages").load("view_count_pages.php",{limit: 25},function(responsive, textStatus, req){
                                      if (textStatus=="error"){
                                        var msg = "Sorry but there was an error: ";
                                        $(".view_count_pages").html(msg + req.status + " " + req.statusText)
                                      }
                                    });
                                } else {
                                    bootbox.alert("Error data: Không thực thi được. Liên hệ với admin");
   ...
   $('#dataTables_pages tbody').on('click', 'a.btn-pinterest', function () {
        // alert("hsjdhsjd");
        var id = $(this).attr("data-id");
