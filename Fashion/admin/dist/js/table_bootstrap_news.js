...
$("#dataTables_news").on("click","a.btn-github",function(e){
    var rpl = $(this);
    var id = $(this).attr("data-id");
    $.ajax({
        type: "POST",
        url: "process_show_news.php",
        data: {id: id},
...
$("#dataTables_news").on("click","a.btn-bitbucket",function(e){
    var rpl = $(this);
    var id = $(this).attr("data-id");
    $.ajax({
        type: "POST",
        url: "process_hide_news.php",
        data: {id: id},
        success: function(response){
            if (response==="success"){
...
$("#model_news_detail").on("show.bs.modal", function(e) {
    var link = $(e.relatedTarget);
    $(this).find(".modal-body").load("load_news_detail.php?id="+link.attr("data-id")); 
});

$(document).ready(function (){
   // Array holding selected row IDs
   var rows_selected = [];
   var table = $('#dataTables_news').DataTable({
      'ajax': 'load_news.php',
      'columnDefs': [{
         'targets': 0,
         'searchable':false,
   ...
   $('#dataTables_news').on('click', 'tbody td:not(:last-child), thead th:first-child', function(e){
   ...
   $('thead input[name="select_all"]', table.table().container()).on('click', function(e){
      if(this.checked){
         $('#dataTables_news tbody input[type="checkbox"]:not(:checked)').trigger('click');
      } else {
         $('#dataTables_news tbody input[type="checkbox"]:checked').trigger('click');
      }
          ...
          bootbox.confirm({ 
            ...
            callback: function(result){
                if (result){
                  ...
                  $.ajax({
                      type: "POST",
                      url: "process_hide_news_selected.php",
                      data: {id: list_id},
                      success: function(response){
                        $(".loader").fadeOut("slow");
                        if (response==="success"){
          ...
          bootbox.confirm({ 
            ...
            callback: function(result){
                if (result){
                  ...
                  $.ajax({
                      type: "POST",
                      url: "process_show_news_selected.php",
                      data: {id: list_id},
                      success: function(response){
                          $(".loader").fadeOut("slow");
                          if (response==="success"){
   ...
   $("#model_news_detail_selected").on("show.bs.modal", function(e) {
        var md = $(this);
        var list_id = [];
        table.rows(".selected",{page:'current'}).every(function(rowIdx, tableLoop, rowLoop){
            var r = this.data();
            ...
            bootbox.confirm({ 
                ...
                callback: function(result){
                    if (result){
                        ...
                        $.ajax({
                            type: "POST",
                            url: "process_delete_news_selected.php",
                            data: {id: list_id},
                            success: function(response){
                                $(".loader").fadeOut("slow");
                                // alert(response);
   ...
   $('#dataTables_news tbody').on('click', 'a.btn-pinterest', function () {
        // alert("hsjdhsjd");
        var id = $(this).attr("data-id");
