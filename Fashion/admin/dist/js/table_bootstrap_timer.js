...
$("#dataTables_timer").on("click","a.btn-github",function(e){
    var rpl = $(this);
    var id = $(this).attr("data-id");
    $.ajax({
        type: "POST",
        url: "process_show_timer.php",
        data: {id: id},
...
$("#dataTables_timer").on("click","a.btn-bitbucket",function(e){
    var rpl = $(this);
    var id = $(this).attr("data-id");
    $.ajax({
        type: "POST",
        url: "process_hide_brand.php",
        data: {id: id},
        success: function(response){
            if (response==="success"){
...
$("#model_timer_detail").on("show.bs.modal", function(e) {
    var link = $(e.relatedTarget);
    $(this).find(".modal-body").load("load_timer_detail.php?id="+link.attr("data-id")); 
});

$(document).ready(function (){
   // Array holding selected row IDs
   var rows_selected = [];
   var table = $('#dataTables_timer').DataTable({
      'ajax': 'load_brand.php',
      'columnDefs': [{
         'targets': 0,
         'searchable':false,
   ...
   // $(".append_button").prepend('<label class="m-t-2"><a class="btn btn-social-icon btn-success m-r-1" href="addtimer.php" title="Thêm hoạt động mới"><i class="fa fa-plus"></i></a> <a class="btn btn-social-icon btn-github m-r-1" id="submit_unchecked_selected" title="Ẩn hoạt động đã chọn"><i class="fa fa-eye-slash"></i></a> <a class="btn btn-social-icon btn-bitbucket m-r-1"  id="submit_checked_selected" title="Cho phép hiển thị các hoạt động đã chọn"><i class="fa fa-eye"></i></a> <a class="btn btn-social-icon btn-pinterest m-r-1" id="submit_delete_selected" title="Xóa các hoạt động đã chọn"><i class="fa fa-trash-o"></i></a></label>');
   
   // Handle click on checkbox
   $('#dataTables_timer tbody').on('click', 'input[type="checkbox"]', function(e){
   ...
   $('#dataTables_timer').on('click', 'tbody td:not(:last-child), thead th:first-child', function(e){
   ...
   $('thead input[name="select_all"]', table.table().container()).on('click', function(e){
      if(this.checked){
         $('#dataTables_timer tbody input[type="checkbox"]:not(:checked)').trigger('click');
      } else {
         $('#dataTables_timer tbody input[type="checkbox"]:checked').trigger('click');
      }
          ...
          bootbox.confirm({ 
            ...
            callback: function(result){
                if (result){
                  ...
                  $.ajax({
                      type: "POST",
                      url: "process_hide_timer_selected.php",
                      data: {id: list_id},
                      success: function(response){
                        $(".loader").fadeOut("slow");
                        if (response==="success"){
   ...
   $("#submit_checked_selected").click(function(e){
        ...
        if (sl===0){
          ...
          bootbox.confirm({ 
            message: "Bạn chắc chắn là muốn hiển thị các hoạt động đã chọn này?", 
            buttons: {
                confirm: {
                    label: 'Đồng ý',
                    className: 'btn-danger'
            ...
            callback: function(result){
                if (result){
                  ...
                  $.ajax({
                      type: "POST",
                      url: "process_show_timer_selected.php",
                      data: {id: list_id},
                      success: function(response){
                          $(".loader").fadeOut("slow");
                          if (response==="success"){
                              table.rows(".selected", {page:'current'}).every(function(rowIdx, tableLoop, rowLoop){
                                  ...
                                  var id = a[0];
                                  var cell = table.cell(this,4);
                                  cell.data("<a class='btn btn-social-icon btn-bitbucket m-t-2 m-r-1' title='Hiển thị' data-id='"+id+"'><i class='fa fa-eye'></i></a> <a class='btn btn-social-icon btn-linkedin m-t-2 m-r-1' title='Chi tiết sản phẩm' data-toggle='modal' data-target='#model_timer_detail' data-id='"+id+"'><i class='fa fa-pencil-square-o'></i></a> <a class='btn btn-social-icon btn-warning m-t-2 m-r-1' title='Sửa thông tin sản phẩm' href='edit_product.php?id="+id+"'><i class='fa fa-pencil'></i></a> <a class='btn btn-social-icon btn-pinterest m-t-2 m-r-1' title='Xóa sản phẩm' data-id='"+id+"'><i class='fa fa-trash-o'></i></a>");
                                  this.invalidate().draw(false);
                              })
                          } else {
                              bootbox.alert("Error data: Không thực thi được. Liên hệ với admin");
   ...
   $("#model_timer_detail_selected").on("show.bs.modal", function(e) {
        var md = $(this);
        var list_id = [];
        table.rows(".selected",{page:'current'}).every(function(rowIdx, tableLoop, rowLoop){
            var r = this.data();
            ...
            bootbox.confirm({ 
                ...
                callback: function(result){
                    if (result){
                        ...
                        $.ajax({
                            type: "POST",
                            url: "process_delete_timer_selected.php",
                            data: {id: list_id},
                            success: function(response){
                                $(".loader").fadeOut("slow");
                                // alert(response);
   ...
   $('#dataTables_timer tbody').on('click', 'a.btn-pinterest', function () {
        // alert("hsjdhsjd");
        var id = $(this).attr("data-id");
        var tr = table.row($(this).parents('tr'));
        bootbox.confirm({ 
            message: "Bạn chắc chắn là muốn xóa hoạt động này?", 
            buttons: {
                confirm: {
                    label: 'Đồng ý',
                    className: 'btn-danger'
