<?php include_once("views/index/v_header.php");?>
              ...
              <div class="box box-info">
                ...
                <div class="box-body pad">
                  <form id="form_add_pages" enctype="multipart/form-data">
                    <div id="infor_name_valid" class="form-group">
                      <input type="hidden" id="infor_id" name="infor_id" class="form-control" value="<?php echo $current_info->infor_id; ?>">
                      <label>Tiêu đề</label>
                      <input type="text" id="infor_name" name="infor_name" class="form-control" placeholder="Ví dụ: Ford Focus 1.6L Trend 5 Cửa" value="<?php echo $current_info->infor_name; ?>">
                    </div>
                    <div id="infor_detail_valid" class="form-group">
                      <label>Nội dung</label>
                      <input type="text" id="infor_detail" name="infor_detail" class="form-control" placeholder="Ví dụ: Ford Focus 1.6L Trend 5 Cửa" onblur="isempty('#infor_detail_valid',this);" value="<?php echo $current_info->infor_detail; ?>">
                    </div>
                    
                    <div class="row">
                      <div class="col-md-4 col-xs-4">
                        <button type="button" onclick="submit_add_pages();" class="btn btn-block btn-success btn-flat">Cập nhật</button>
                      </div>
                      <div class="col-md-4 col-xs-4">
                        <button type="button" class="btn btn-block btn-warning btn-flat" onclick="reset_form_pages();">Hủy</button>
                      </div>
                      <div class="col-md-4 col-xs-4">
                        <button type="button" class="btn btn-block btn-default btn-flat" onclick="location.href='info.php'">Xem List</button>
                      </div>
                    </div>
                  </form>
                </div>
    ...
    <script>
        ...
        $.ajax({
            ...
            success: function(response){
              $(".loader").fadeOut("slow");
               // alert(response);
              if(response==="success"){
                  bootbox.alert("Cập nhật trang thành công");
                 // $('#form_add_product')[0].reset();
                  ...
                  $("#infor_detail_valid").addClass('has-error');
      ...
      function isvalid(){
        flag = true;
        if($("#infor_name").val().trim()===""){
          $("#infor_name_valid").addClass('has-error');
          flag = false;
        } 
        if ($("#infor_detail").val().trim()===""){
          $("#infor_detail_valid").addClass('has-error');
          flag = false;
        }
      ...
      function isempty(id,obj){
        if($(obj).val().trim()===""){
          ...
          if(id==="#infor_name_valid"){
            $("#infor_detail").val(slug($(obj).val().trim()));
          }
        }
      }
