      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          ...
          <ol class="breadcrumb">
            ...
            <li><a href="listbrand.php">Hiệu xe</a></li>
              ...
              <div class="box box-info">
                ...
                <div class="box-body pad">
                  <form id="form_add_timer" enctype="multipart/form-data">
                    <div id="brand_name_valid" class="form-group">
                      <input type="hidden" id="brand_id" name="brand_id" class="form-control" value="<?php echo $current_brand->brand_id; ?>">
                      <label>Hiệu xe</label>
                      <input type="text" id="brand_name" name="brand_name" class="form-control" placeholder="Ví dụ: Ford" onblur="isempty('#brand_name_valid',this);" value="<?php echo $current_brand->brand_name; ?>">
                    </div>
                    <div id="brand_slug_valid" class="form-group">
                      <label>Tên đường dẫn</label>
                      <input type="hidden" id="brand_slug_src" name="brand_slug_src" class="form-control" value="<?php echo $current_brand->brand_slug;?>">
                      <input type="text" id="brand_slug" name="brand_slug" class="form-control" placeholder="Ví dụ: Ford Focus 1.6L Trend 5 Cửa" onblur="isempty('#brand_slug_valid',this);" value="<?php echo $current_brand->brand_slug; ?>">
                    </div>
    ...
    <script>
        ...
        $.ajax({
            ...
            success: function(response){
              $(".loader").fadeOut("slow");
               // alert(response);
              if(response==="success"){
                  bootbox.alert("Cập nhật hoạt động thành công");
                 // $('#form_add_product')[0].reset();
                  setTimeout(function(){window.location.reload(1);}, 1500);
              } else if (response==="duplicate") {
                  bootbox.alert("Tên hoạt động hoặc đường dẫn đã bị trùng. Vui lòng thay đổi tên hoặc đường dẫn khác");
                  $("#brand_slug_valid").addClass('has-error');
      ...
      function isvalid(){
        ...
        if($("#brand_name").val().trim()===""){
          $("#brand_name_valid").addClass('has-error');
          flag = false;
        } 
        if ($("#brand_slug").val().trim()===""){
          $("#brand_slug_valid").addClass('has-error');
          flag = false;
        }
        return flag;
      ...
      function isempty(id,obj){
        if($(obj).val().trim()===""){
          ...
          if(id==="#brand_name_valid"){
            $("#brand_slug").val(slug($(obj).val().trim()));
          }
        }
      }
